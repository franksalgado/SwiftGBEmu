...
func GetTitle(romData: [U8]) -> String {
...
func CreateCartridgeHeader(romData: [U8]) -> CartridgeHeader {
    return CartridgeHeader(title: GetTitle(romData: romData),
...
func CreateCartridgeEntry(fileURL: URL) -> Cartridge? {
    ...
    let cartridgeHeader = CreateCartridgeHeader(romData: romData);
    return Cartridge(fileURL: fileURL, cartridgeHeader: cartridgeHeader, romData: romData);
}
...
func CreateCartridgeState(romData: [U8]) -> CartridgeState {
    return CartridgeState(romData: romData);
}
